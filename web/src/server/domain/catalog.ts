...
function getDefaultWebsiteTemplateHtmlPreviewProducts(): ManagedProductRecord[] {
  return listWebsiteTemplateHtmlPreviews().map((template) => {
    ...
    return withParentTaxonomy({
      ...
      tag: "Preview",
      published: true,
      teaser: template.teaser ?? `Website preview for the ${template.title} website template.`,
      summary: template.summary ?? `Preview-first ${template.title} template with website preview and shop-ready routing.`,
      audience: "Buyers who want to inspect a website preview before purchasing template packages.",
      features: [
        "Iframe-safe in-frame website preview",
      ...
      inScope: ["Template package", "In-frame preview", "Category routing"],
      outOfScope: ["Custom development", "Complex integrations"],
      enhancementPlan: ["Expand website preview catalog", "Connect additional template files"],
      stack: ["HTML5", "CSS3", "JavaScript"],
      variants: [
        {
          slug: "standard",
      ...
      highlights: [
        { label: "Preview", value: "Website preview" },
