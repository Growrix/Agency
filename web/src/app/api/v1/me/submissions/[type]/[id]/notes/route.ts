...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!isReplyCapableType(submissionType)) {
      throw new ApiError(
        "FORBIDDEN",
        403,
        "Customer replies are only supported on service requests and support threads.",
      );
    }
    ...
    return errorResponse(
      error instanceof Error ? error : new ApiError("INTERNAL_ERROR", 500, "Unable to add reply."),
    );
  }
}
