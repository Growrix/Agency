...
export function ChatLauncher() {
    ...
    <>
      <div className="flex items-center justify-between bg-[var(--color-contrast)] px-4 py-3 text-[var(--color-contrast-text)]">
        <div className="flex items-center gap-2">
          <span className={cn("size-2 rounded-full bg-emerald-400", reduced ? null : "animate-pulse")} aria-hidden />
  ...
  const icon = open ? <XMarkIcon className="size-6" /> : <ChatBubbleLeftRightIcon className="size-6" />;

  if (reduced) {
    return (
      <div className="hidden lg:block fixed bottom-6 right-6 z-50">
        {open && (
          <div className="mb-3 w-[340px] rounded-[20px] border border-[var(--color-border)] bg-[var(--color-surface)] shadow-[var(--shadow-3)] overflow-hidden">
            {panelInner}
          </div>
        )}
        <button
