...
async function buildDocumentPayload(client, contentType, data, dryRun) {
  ...
  if (contentType === "shopItem") {
    ...
    return {
      ...
      livePreviewUrl: String(data?.livePreviewUrl ?? "").trim() || undefined,
      embeddedPreviewUrl: String(data?.embeddedPreviewUrl ?? "").trim() || undefined,
      categoryLabel: String(data?.categoryLabel ?? "").trim() || "HTML Templates",
      categorySlug: String(data?.categorySlug ?? "").trim() || "html-templates",
      type: String(data?.type ?? "").trim() || "Email Templates",
      typeSlug: String(data?.typeSlug ?? "").trim() || "email-templates",
      industry: String(data?.industry ?? "").trim() || "Digital Products",
      industrySlug: String(data?.industrySlug ?? "").trim() || "digital-products",
