...
- Active implementation session:
  ...
  - removed a booking-form hydration mismatch caused by SSR time-derived minimum-slot values, hardened JS-dependent booking and live-chat forms against pre-hydration native submits, and switched Vitest scripts to the runner config loader while investigating remaining Windows-specific path-resolution friction in local unit/integration execution
